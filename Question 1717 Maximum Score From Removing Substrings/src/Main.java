...
public class Main {
    ...
    private static String removeSubstring(String input, String targetPair) {
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            if (currentChar == targetPair.charAt(1) && !charStack.isEmpty() && charStack.peek() == targetPair.charAt(0)) charStack.pop();
            else charStack.push(currentChar);
        }
        StringBuilder remainingChars = new StringBuilder();
        while (!charStack.isEmpty()) remainingChars.append(charStack.pop());
