public class Main {
    public static String numberToWords(int num) {
        ...
        num = num/1000;
        if(num > 0 && num%1000>0) result = numberToWordsHelper(num%1000) + "Thousand " + result;
        num = num/1000;
        if(num > 0 && num%1000>0) result = numberToWordsHelper(num%1000) + "Million " + result;
        num = num/1000;
        if(num > 0) result = numberToWordsHelper(num%1000) + "Billion " + result;
        return result.trim();
    }
    ...
    public static String numberToWordsHelper(int num){
        ...
        if(num < 20 && num > 9) result += teenString[num%10]+" ";
            ...
            if(num > 19) result += tenString[num/10]+" ";
            num = num % 10;
