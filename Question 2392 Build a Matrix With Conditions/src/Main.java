...
public class Main {
    public int[][] buildMatrix(int k, int[][] rowConditions, int[][] colConditions) {
        ...
        if (row_sorting.isEmpty() || col_sorting.isEmpty()) return new int[0][0];
        Map<Integer, int[]> value_position = new HashMap<>();
        for (int n = 1; n <= k; ++n) value_position.put(n, new int[2]);
        for (int ind = 0; ind < row_sorting.size(); ++ind) value_position.get(row_sorting.get(ind))[0] = ind;
        for (int ind = 0; ind < col_sorting.size(); ++ind) value_position.get(col_sorting.get(ind))[1] = ind;
        int[][] res = new int[k][k];
